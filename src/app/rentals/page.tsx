...
export default function RentalsPage() {
  ...
  const loadMoreRentals = useCallback(async () => {
    ...
    try {
      ...
      if (lastDoc) {
        ...
        q = query(collection(db, 'rentals'), orderBy('createdAt', 'desc'), limit(20));
      }

      const snap = await getDocs(q);
      const newRentals = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      
      if (newRentals.length < 20) {
        setHasMore(false);
      }
  ...
  // Fetch initial rentals with caching
  useEffect(() => {
    const fetchRentals = async () => {
      ...
      setLoading(true);
      try {
        const q = query(collection(db, 'rentals'), orderBy('createdAt', 'desc'), limit(20));
        const snap = await getDocs(q);
  ...
  // Render rental item for progressive list
  const renderRentalItem = useCallback((rental: any, index: number) => (
    <div className="col-12 col-md-6 col-lg-4">
      <CarCard car={rental} />
    </div>
  ), []);

  // Key extractor for rentals
  const keyExtractor = useCallback((rental: any, index: number) => rental.id || `rental-${index}`, []);
